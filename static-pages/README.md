# Static Pages

This folder contains starter markdown pages for cards currently defined in:

- `/writings.html`
- `/experimental-sandbox.html`

## Writings

- `writings/the-density-of-the-present.md`
- `writings/platform-logic-and-the-grammar-of-appearance.md`
- `writings/forlagsdeckaren.md`
- `writings/shortstory-vm-1.md`
- `writings/shortstory-vm-4.md`
- `writings/shortstory-vm-5.md`
- `writings/shortstory-vm-6.md`
- `writings/the-body-without-coordinates.md`
- `writings/against-the-archive-impulse.md`
- `writings/notes-on-disappearance.md`
- `writings/psychotechnical-subjectivity-a-field-note.md`
- `writings/what-the-interface-withholds.md`
- `writings/authenticity-after-instagram.md`
- `writings/correspondence-persons-and-machines.md`

## Experimental Sandbox

- `experimental-sandbox/recursive-time-capsules.md`
- `experimental-sandbox/the-avatar-problem.md`
- `experimental-sandbox/mediated-existence-studies.md`
- `experimental-sandbox/psychotechnical-subjectivity-working-glossary.md`
- `experimental-sandbox/infrastructural-publishing-logic.md`
- `experimental-sandbox/corpus-of-departures.md`
- `experimental-sandbox/digital-identity-and-bodily-integrity.md`
- `experimental-sandbox/text-systems-that-talk-back.md`
- `experimental-sandbox/cultural-diagnostics-a-survey.md`
- `experimental-sandbox/the-attention-audit.md`
